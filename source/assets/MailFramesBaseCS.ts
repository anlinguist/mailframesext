export abstract class MailFrames {
    ...
    addButtons() {
        let targets = this.buttonTargets
    ...
    init() {
        this.observeDOM();
        this.observeChromeMessages();
    }

    observeChromeMessages() {
        ...
        this.observer = new MutationObserver((mutations) => {
            mutations.forEach((mutation) => {
                if (mutation.type === "childList" && mutation.addedNodes.length > 0) {
                    this.addCopyButton();
                    this.injectTarget = this.getInjectTarget();
                }
            });
        });
    ...
    addCopyButton() {
        this.buttonTargets = this.getButtonTargets();
        this.removeButtons();
        this.addButtons();
    }

    updateDraft(message: any) {
        this.injectTarget.innerHTML = message.body;
        this.injectTarget.dispatchEvent(new Event('input', { bubbles: true }));
    }

    abstract getButtonTargets(): any;
    abstract getInjectTarget(): any;
}